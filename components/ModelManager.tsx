...
export const ModelManager: React.FC<ModelManagerProps> = ({
  ...
  const groupedModels = {
      chat: models.filter(m => m.type === 'chat' || m.type === 'nlp' as any || !m.type || m.type === 'multimodal'),
      ...
      <div key={m.id} className={`border-2 border-black p-2 mb-2 flex justify-between items-center group bg-white/50 ${isEditing ? 'bg-blue-100/50 border-blue-500' : 'hover:bg-black/5'}`}>
        <div>
          <div className="font-bold flex items-center gap-2">
              <span className="w-5 h-5 opacity-70">{parent ? getProviderIcon(parent.type) : '🔧'}</span>
              {m.name}
              {m.isDefault && (
              ...
              {m.type === 'multimodal' && (
                  <PixelBadge theme={theme} color="bg-pink-400 text-black">VL</PixelBadge>
              )}
          </div>
          <div className="text-xs opacity-70 ml-7">{parent?.name} / {m.modelId}</div>
          <div className="text-[10px] opacity-50 mt-1 ml-7 font-mono">
              ...
              {activeTab === 'providers' ? (
                providers.map(p => (
                  <div key={p.id} className={`border-2 border-black p-2 mb-2 flex justify-between items-center group ${editingProviderId === p.id ? 'bg-blue-100/50 border-blue-500' : 'hover:bg-black/5'}`}>
                    <div>
                      <div className="font-bold flex items-center gap-2">
                        <span className="w-5 h-5">{getProviderIcon(p.type)}</span>
                        {p.name}
                      </div>
                      <div className="text-xs opacity-70 ml-7">{p.baseUrl}</div>
                    </div>
                    <div className="flex items-center">
                   ...
                   {groupedModels.chat.length > 0 && (
                      <div>
                          <div className="flex items-center gap-2 mb-2">
                               <PixelBadge theme={theme} color={getModelTypeColor('chat')}>CHAT / VL</PixelBadge>
